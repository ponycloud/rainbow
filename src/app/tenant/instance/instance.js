...
(function() {
  ...
  module.controller('InstanceWizardCtrl', InstanceWizardCtrl = (function() {
    InstanceWizardCtrl.$inject = ['$scope', '$routeParams', '$modal', '$location', 'TenantInstance', 'TenantSwitchNetwork', 'TenantInstanceVdisk', 'TenantInstanceVnic', 'TenantSwitch', 'Tenant', 'TenantInstanceVnicAddress', 'TenantVolume', 'StoragePool', 'Switch', 'SwitchNetwork', 'CpuProfile', 'TenantAffinityGroup', 'TenantImage', 'Image', 'dataContainer', 'WizardHandler'];

    function InstanceWizardCtrl($scope, $routeParams, $modal, $location, TenantInstance, TenantSwitchNetwork, TenantInstanceVdisk, TenantInstanceVnic, TenantSwitch, Tenant, TenantInstanceVnicAddress, TenantVolume, StoragePool, Switch, SwitchNetwork, CpuProfile, TenantAffinityGroup, TenantImage, Image, dataContainer, WizardHandler) {
      var lookupSwitch, setDefaultVolumeSize;
      $scope.instance = {
        uuid: 'instance_uuid',
        selectedVolumes: [],
        vdisks: [],
      ...
      lookupSwitch = function(switchUUID) {
        var switchItem, _i, _len, _ref;
        _ref = $scope.switches;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          switchItem = _ref[_i];
      ...
      $scope.getSwitchType = function(item) {
        if (!item.desired.tenant) {
          return 'Public';
        }
        return 'Private';
      };
      ...
      $scope.createInstance = function() {
        ...
        return Tenant.patch(params, patch, function(response) {
          console.log(response);
          return $location.path($routeParams.tenant + '/instance/' + response.uuids.instance_uuid);
        });
      };
    }
